...
@JavaConcept(
    name = "Generics Wildcards (PECS - Producer Extends Consumer Super)",
    difficulty = Difficulty.ADVANCED,
    what = "Java Generics are invariant, meaning List<Integer> is not a subclass of List<Number>. Wildcards (?) resolve this constraint. PECS dictates: use '? extends T' when reading elements from a structure (Producer), and '? super T' when writing elements to a structure (Consumer).",
    whyItMatters = "Understanding PECS is essential for designing flexible API methods. Failing to apply wildcards prevents clients from passing collections of subclass types to read-only parameters, making APIs rigid.",
    keyPoints = {
    ...
    interviewQuestions = {
        @Question(
            ...
            answer = "PECS stands for Producer Extends, Consumer Super. Use '? extends T' if your collection produces items (you get items out of it). Use '? super T' if your collection consumes items (you put items into it)."
        ),
        @Question(
            question = "Why can't you add elements to a List<? extends Number>?",
            answer = "Because the compiler doesn't know what specific concrete type the list holds. It could be List<Integer>, List<Double>, or List<Float>. To prevent adding a Double into a list of Integers, the compiler blocks all additions (except null)."
        )
    },
    pitfalls = {
...
public class GenericsWildcards {
    public static void main(String[] args) {
        ...
        // 1. Invariance Demo
        ConsoleFormatter.printStep("Invariance Contradiction", "Validating that List<Integer> does not assign to List<Number>");
        List<Integer> intList = List.of(1, 2, 3);
        // List<Number> numList = intList; // DOES NOT COMPILE!
        System.out.println("  Verified: List<Integer> cannot be assigned directly to List<Number> reference (Invariance).");
